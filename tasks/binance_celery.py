...
def load(lines):
    ...
    conn = psycopg2.connect(host="192.168.1.251", user="postgres", password="secret", port=5432)
    cursor = conn.cursor()
    args_str = b','.join(cursor.mogrify("%s", (x, )) for x in data)
    cursor.execute(f"INSERT INTO ohlcv (open, high, low, close, volume, pair, interval, datetime, exchange) VALUES " + args_str.decode('utf8'))
    conn.commit()
    cursor.close()
    conn.close()
